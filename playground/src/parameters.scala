...
object decode_config extends DeType with ALUOP with BrType with Insts with MemMode{
    ...
    val decodeTable = Array(   
        ...
        MUL    -> List(RType, alu_MUL,  mode_NOP, true.B, NO_JMP),
        MULH   -> List(RType, alu_MULH, mode_NOP, true.B, NO_JMP),
        MULHU  -> List(RType, alu_MULHU,mode_NOP, true.B, NO_JMP),
        DIV    -> List(RType, alu_DIV,  mode_NOP, true.B, NO_JMP),
        DIVU   -> List(RType, alu_DIVU, mode_NOP, true.B, NO_JMP),
        REM    -> List(RType, alu_REM,  mode_NOP, true.B, NO_JMP),
        REMU   -> List(RType, alu_REMU, mode_NOP, true.B, NO_JMP),
                                                //|write-reg|跳转信号
